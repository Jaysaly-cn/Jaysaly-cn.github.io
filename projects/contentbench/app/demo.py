"""Adapted from portfolio MeetingActions / FeedbackLens MIT demo. Single process, isolated databases."""
import asyncio
import os
import re
import secrets
...
def create_demo_app(root=None, clock=time.monotonic):
    ...
    def clean():
        ...
        for path in folder.glob('cbdemo-*.sqlite3'):
            if re.fullmatch(r'cbdemo-[0-9a-f]{24}-[0-9a-f]{32}\.sqlite3',path.name) and path not in active_paths:
    ...
    def seed(path):
        initialize(path)
        brief={'name':'拾光任务本 · 合成演示','audience':'职场新人','objective':'邀请体验任务整理功能','tone':'清晰、克制、友好',
               'facts':[{'id':'F1','text':'支持手动添加任务和设置截止日期','source':'合成产品说明 v1'},
                        {'id':'F2','text':'支持按周查看任务清单','source':'合成产品说明 v1'}],
               'forbidden':['行业第一','保证','最强'],'required_phrase':'示例产品，仅供学习'}
        cid=uuid4().hex
        with connect(path) as db:
            db.execute('INSERT INTO campaigns VALUES(?,?,?)',(cid,dump(brief),now()))
            db.execute('INSERT INTO events(campaign_id,action,detail,created_at) VALUES(?,?,?,?)',
                       (cid,'campaign_created','合成演示初始化',now()))
    ...
    def charge_model():
        ...
        if session['models']>=MODEL_CALLS or len(model_times)>=24:
            raise HTTPException(429,'免费演示模型额度暂时用完；仍可手工录稿、修订、审核和导出')
        session['models']+=1
    ...
    async def sandbox(request,call_next):
        ...
        allowed=os.getenv('CB_DEMO_HOST','')
        ...
        if path=='/demo/status':
            return JSONResponse({'service':'ContentBench temporary demo','temporary':True,'session_minutes':TTL//60})
        ...
        if not session or session['expires']<=stamp:
            ...
            database=folder/f'cbdemo-{namespace}-{secrets.token_hex(16)}.sqlite3'
            seed(database)
        ...
        try:
            if path=='/':
                ...
                banner='<section class="demo-banner" role="note"><strong>临时公开演示 · 每位访客独立空间</strong><p>已放入合成品牌资料。实验性小模型可能编造功能、漏说明；请逐句核对，修订并审核后冻结交付。请仅使用合成资料，输入在演示服务器处理。会话30分钟，最多3次模型尝试，失败也计入；仍可手工录稿与修订。可下载JSON或Markdown冻结包。重启后会话失效；入口依赖开发机在线，不是稳定云服务。</p><a href="/">会话过期后重新进入 →</a></section>'
