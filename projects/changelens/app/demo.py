...
def create_demo_app(root=None, clock=time.monotonic):
    ...
    def clean():
        ...
        for path in folder.glob('cldemo-*.sqlite3'):
            if re.fullmatch(r'cldemo-[0-9a-f]{24}-[0-9a-f]{32}\.sqlite3',path.name) and path not in active_paths:
    ...
    def seed(path):
        ...
        from .diff import digest
        conn=sqlite3.connect(path)
        try:
            with conn:
                did=uuid4().hex
                conn.execute('INSERT INTO documents VALUES(?,?,?)',(did,'导出规则 · 合成示例',now()))
                for number,label,text in [(1,'初版','仅管理员允许导出。\n文件保留7天。\n'),(2,'修订版','仅管理员允许导出。\n文件保留3天。\n新增审计记录。\n')]:
                    conn.execute('INSERT INTO versions VALUES(?,?,?,?,?,?,?)',(uuid4().hex,did,number,label,text,digest(text),now()))
        finally:
            conn.close()
    ...
    async def sandbox(request,call_next):
        ...
        allowed=os.getenv('CL_DEMO_HOST','')
        ...
        if path=='/demo/status':
            return JSONResponse({'service':'ChangeLens temporary demo','temporary':True,'session_minutes':TTL//60})
        ...
        if not session or session['expires']<=stamp:
            ...
            database=folder/f'cldemo-{namespace}-{secrets.token_hex(16)}.sqlite3'
            seed(database)
        ...
        if request.method not in ('GET','HEAD'):
            ...
            session['writes']+=1
        if request.method=='POST' and re.fullmatch(r'/api/comparisons/[^/]+/operations/[^/]+/suggest/?',path):
            while model_times and stamp-model_times[0]>3600:
                model_times.popleft()
            if session['models']>=MODEL_CALLS or len(model_times)>=24:
                return JSONResponse({'detail':'免费演示模型额度暂时用完；仍可手工说明、审核与导出'},status_code=429)
        ...
        try:
            if path=='/':
                ...
                banner='<aside class="demo-banner" role="note"><strong>临时公开演示 · 每位访客独立空间</strong><p>已放入同一文档的两个合成版本。选择“导出规则”后点击“比较版本”，可使用 AI 或手工说明，审核后冻结简报。请仅使用合成资料，资料在演示服务器处理。会话30分钟后失效并清理，重启也会失效；最多3次免费模型请求（失败也占额度），单次输入最多40KB。请及时导出。入口依赖开发机在线，尚非稳定服务。</p><a href="/">会话过期后重新进入 →</a></aside>'
