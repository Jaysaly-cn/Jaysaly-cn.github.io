...
资料目录可修订标题、正文、来源与使用范围，必须填写至少5字修改说明。保存时携带当前version，版本过期返回409；无变化返回422。每份资料最多100个历史版本。保存期间锁定资料表单；未保存内容时不能直接改选其他资料，支持清空/取消。

PATCH /api/documents/{id} 接收原资料字段及version/change_note；GET /api/documents/{id}/history 返回current和versions，每条历史保留原始资料快照、修改说明和记录时间。删除后current为空，历史仍可按ID查询；这不是彻底删除机制。重新载入已删除示例会继续版本号，不能覆盖旧历史。

旧数据库初始化只记录当前版本并注明“更早历史不可恢复”，不会编造过去版本。新增导入、合成示例和工单审核发布均写入初始历史。改变使用范围影响后续检索，旧回答、已发布工单快照和旧复查不改写；范围依旧不是用户权限控制。

当前30测试通过，新增覆盖旧库幂等迁移、修订冲突、旧回答保持、新版本复查、删除/重载历史及访客隔离。公网浏览器完成合成退款政策7→14天修订，手机视口展开v1/v2，复查展示旧v1与新v2，见 artifacts/document-version-browser.json。没有据此宣称真实政策或业务成果。
