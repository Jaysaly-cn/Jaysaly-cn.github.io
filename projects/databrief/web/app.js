...
action($('#bundle'),async()=>{
 if(!shown||shown.result.state!=='succeeded')return;
 const id=shown.id;
 const response=await fetch('/api/runs/'+id+'/bundle',{headers:{Authorization:'Bearer '+(sessionStorage.getItem('dbr-token')||'')}});
 if(!response.ok){const error=await response.json();throw Error(error.detail||'结果包导出失败');}
 const url=URL.createObjectURL(await response.blob()),a=el('a');a.href=url;a.download=`databrief-${id}.zip`;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
 notice('结果包已交给浏览器下载。CSV空值为\\N、公式前缀文本已加单引号；请以JSON核对原值与截断标记。');
});
