...
from .data import initialize, import_csv

COOKIE = 'dbr_demo_session'
TTL = 1800
MAX_SESSIONS = 12
MAX_WRITES = 60
MODEL_CALLS = 3
...
def create_demo_app(root=None, clock=time.monotonic):
    ...
    def clean():
        ...
        for path in folder.glob('dbrdemo-*.sqlite3'):
            if re.fullmatch(r'dbrdemo-[0-9a-f]{24}-[0-9a-f]{32}\.sqlite3',path.name) and path not in active_paths:
    ...
    def seed(path):
        initialize(path)
        import_csv(path,'合成渠道数据',(ROOT/'samples/channels.csv').read_bytes())

    @app.middleware('http')
    async def sandbox(request,call_next):
        path=request.url.path
        host=request.url.hostname or ''
        allowed=os.getenv('DBR_DEMO_HOST','')
        ...
        if path=='/demo/status':
            return JSONResponse({'service':'DataBrief temporary demo','temporary':True,'session_minutes':TTL//60})
        ...
        if not session or session['expires']<=stamp:
            ...
            database=folder/f'dbrdemo-{namespace}-{secrets.token_hex(16)}.sqlite3'
            seed(database)
        ...
        try:
            if path=='/':
                ...
                banner='<div class="demo-banner" role="note"><strong>临时公开演示 · 每位访客独立空间</strong><p>已放入合成渠道数据。请选择数据集开始；无需模型也能使用可视化查询。请仅使用合成资料，会话30分钟后失效并清理，重启也会失效。每会话最多3次免费模型请求，CSV输入最多40KB；结果请及时导出。此地址依赖开发机在线，不保证持续可用。</p><a href="/">会话过期后重新进入 →</a></div>'
