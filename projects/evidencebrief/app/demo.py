"""Single-process, temporary public sandbox; never opens the private workspace DB."""
import asyncio
import hashlib
import json
import os
...
from .main import create_app, ROOT
from .store import initialize, connect, now

COOKIE = 'eb_demo_session'
TTL = 1800
MAX_SESSIONS = 12
MAX_WRITES = 60
MODEL_CALLS = 3
...
def create_demo_app(root=None, clock=time.monotonic):
    ...
    def clean():
        ...
        for path in folder.glob('ebdemo-*.sqlite3'):
            if re.fullmatch(r'ebdemo-[0-9a-f]{24}-[0-9a-f]{32}\.sqlite3',path.name) and path not in active_paths:
    ...
    def seed(path):
        initialize(path)
        pid,sid=secrets.token_hex(16),secrets.token_hex(16)
        text='Alpha 团队版每月 19 元，支持导入 Markdown 文档。免费版不支持团队协作。本材料为合成演示。'
        with connect(path) as db:
            db.execute('INSERT INTO projects VALUES(?,?,?,?,?,?)',(pid,'合成知识库选型','核对套餐价格与导入格式',
                       json.dumps(['Alpha','Beta']),json.dumps(['价格','导入格式']),now()))
            db.execute('INSERT INTO sources VALUES(?,?,?,?,?,?,?,?,?,?,?,0)',
                       (sid,pid,'Alpha','合成产品说明','',text,'paste',hashlib.sha256(text.encode()).hexdigest(),'',now(),''))

    @app.middleware('http')
    async def sandbox(request,call_next):
        path=request.url.path
        host=request.url.hostname or ''
        allowed=os.getenv('EB_DEMO_HOST','')
        ...
        if path=='/demo/status':
            return JSONResponse({'service':'EvidenceBrief temporary demo','temporary':True,'session_minutes':TTL//60})
        ...
        if not session or session['expires']<=stamp:
            ...
            database=folder/f'ebdemo-{namespace}-{secrets.token_hex(16)}.sqlite3'
            seed(database)
        ...
        try:
            ...
            if path.endswith('/suggest'):
                ...
                if session['models']>=MODEL_CALLS or len(model_times)>=24:
                    return JSONResponse({'detail':'免费演示模型额度暂时用完；仍可手工核对和生成报告'},status_code=429)
                session['models']+=1;model_times.append(stamp)
            if path=='/':
                html=(ROOT/'web/index.html').read_text(encoding='utf-8')
                banner='<div class="demo-banner" role="note"><strong>临时公开演示 · 每位访客独立空间</strong><p>已放入合成材料。请勿输入个人或公司机密；会话30分钟后失效并清理，重启也会失效。每会话最多3次免费模型请求，外部采集已关闭。此地址依赖开发机在线，不保证持续可用。</p><a href="/">会话过期后重新进入 →</a></div>'
                response=HTMLResponse(html.replace('<body>','<body class="demo-mode">'+banner))
