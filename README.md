...
**ToC 日用 AI 第二辑：** [六款实用工具](https://jaysaly-cn.github.io/everyday/) · [源码与验证记录](everyday/DELIVERY.md)。图片识别、去背景、语音字幕、英译中、餐单与日程，均在浏览器真实运行模型，区别于上方的 ToB 合成工作流演示。
...
视觉任务使用项目仓库中的真实样例数据；蛋白任务仅对内置测试样例展示已知标签。农业世界模型与植保智能体页面是基于真实项目机制设计的交互产品原型。真实模型推理仍需独立部署 API，这些演示不在 GitHub Pages 上运行后端、数据库或存储密钥。另设的 everyday 工具在浏览器运行模型，部分权重随静态资源分发。
